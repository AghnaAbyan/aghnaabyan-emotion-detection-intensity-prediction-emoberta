...
class EmotionDataset(Dataset):
  ...
  def __getitem__(self, idx):
    ...
    if self.augment and self.labels is not None: # Augmentation
      if not np.isnan(self.labels[idx]).any():
    ...
    if self.labels is not None:
      if np.isnan(self.labels[idx]).any():
        label_mask = ~np.isnan(self.labels[idx])
        item['label_mask'] = torch.tensor(label_mask, dtype=torch.float)
        labels = np.nan_to_num(self.labels[idx], 0)
      else:
        item['label_mask'] = torch.ones(len(self.labels[idx]), dtype=torch.float)
        labels = self.labels[idx]

      item['labels'] = torch.tensor(labels, dtype=torch.float)
      item['label_mask'] = item['label_mask'].to(torch.float)
...
class MultiLabelEmotionModel(nn.Module):
  ...
  def forward(self, input_ids, attention_mask, features, labels=None, label_mask=None):
    ...
    logits = self.regressor(combined_features) # Regressor

    loss = None
    if labels is not None and label_mask is not None:
      # Masked MSE
      mse_loss = nn.MSELoss(reduction='none')(logits, labels)
      masked_mse_loss = (mse_loss * label_mask).sum() / (label_mask.sum() + 1e-8)
      # L1 loss
      l1_loss = nn.L1Loss(reduction='none')(logits, labels)
      masked_l1_loss = (l1_loss * label_mask).sum() / (label_mask.sum() + 1e-8)
      loss = masked_mse_loss + 0.1 * masked_l1_loss
...
def train_model(config, train_data, val_data=None):
  ...
  train_labels = train_data[config.EMOTION_NAMES].values
  ...
  for epoch in range(config.EPOCHS):
    ...
    if val_data is not None:
      ...
      if all_labels:
        ...
        for i, emotion in enumerate(config.EMOTION_NAMES):
          emotion_mask = valid_mask[:, i]
          if emotion_mask.any():
            correlation = pearsonr(
              all_labels[emotion_mask, i],
              all_preds[emotion_mask, i]
            )[0]
            print(f"{emotion} correlation: {correlation:.4f}")

      if avg_train_loss < best_val_loss:
        best_val_loss = avg_train_loss
        torch.save(model.state_dict(), 'model_track_b.pt')
...
if __name__ == "__main__":
    ...
    set_seed(42)
    # Load data
    train_data = pd.read_csv('/home/asrock/QNLP/abyanproject/Track_B_Train_Dev_Combined.csv')
    test_data = pd.read_csv('/home/asrock/QNLP/abyanproject/Test_Eng_B.csv')
    ...
    numeric_cols = test_results.select_dtypes(include=['float', 'int']).columns
    test_results[numeric_cols] = test_results[numeric_cols].round().astype(int)
    if 'text' in test_results.columns:
        test_results['text'] = test_results['text'].astype(str).apply(lambda x: f'"{x}"' if not x.startswith('"') and not x.endswith('"') else x)
    test_results.to_csv('/home/asrock/QNLP/abyanproject/Waste/track_b_testing/05-07/5-7-2025_3.csv', index=False)

    # Create submission file without id column
    df_operation = test_results.drop(['id'], axis=1)
    output_path = '/home/asrock/QNLP/abyanproject/Use/track_b_testing/05-07/5-7-2025_3.csv'
